...
- **Content Hash Deduplication**: Uses SHA-256 content indexing (`images.content_hash`) to instantly reuse metadata for duplicate image uploads per user, bypassing redundant queue and AI processing, with transactional race condition safety and R2 orphan cleanup.
...
- **Security & Comprehensive Rate Limiting**: Hardened with Helmet HTTP headers, XSS payload sanitization, UUID route validation, 1MB JSON limits, and tiered rate limiting across auth (signup, login, google, refresh), upload, retry, and global API fallback.
...
- **SigNoz Observability & Queue Depth Metrics**: OpenTelemetry instrumentation exporting traces, GenAI model token usage metrics, authentication metrics, and real-time BullMQ queue depth polling (`ampm.queue.depth`) to SigNoz.
